	...
	: QMainWindow(parent)
{
	...
	this->trainview->style = 0;
	this->trainview->isrun = false;

	this->trainview->m_pTrack->speed = 50;
	this->trainview->m_pTrack->subdivision = 600.f;
	this->trainview->m_pTrack->tension = 1./2.;
	this->trainview->m_pTrack->sleeperFrqt = 10.f;
	this->trainview->m_pTrack->standCarIndex = -1;
	this->trainview->m_pTrack->sleeperVisible = true;
	this->trainview->m_pTrack->physics = true;
	...
	connect(ui.comboStyle	, SIGNAL(currentIndexChanged(QString)), this, SLOT(ChangeTrackStyle(QString)));
	connect(ui.aClassicTrain, SIGNAL(triggered()), this, SLOT(ChangeStyleToClassic()));
	connect(ui.aSimple		, SIGNAL(triggered()), this, SLOT(ChangeStyleToSimple()));
	connect(ui.aRollerCoaster		, SIGNAL(triggered()), this, SLOT(ChangeStyleToRoller()));

	connect( ui.bPlay		,SIGNAL(clicked()),this,SLOT(SwitchPlayAndPause())				);
	connect( ui.moveBackword,SIGNAL(clicked()),this,SLOT(MoveBackWord())				);
	connect( ui.moveForword ,SIGNAL(clicked()),this,SLOT(MoveForWord())				);
	connect( ui.addCar		,SIGNAL(clicked()),this,SLOT(AddCar())				);
	connect( ui.rmCar		,SIGNAL(clicked()),this,SLOT(RemoveCar())				);
	connect( ui.mvForward	,SIGNAL(clicked()),this,SLOT(MVViewPosForward())				);
	connect( ui.mvBackword	,SIGNAL(clicked()),this,SLOT(MVViewPosBackward())				);
	connect( ui.sSpeed		,SIGNAL(valueChanged(int)),this,SLOT(ChangeSpeedOfTrain(int))	);
	connect( ui.drawSleeper	,SIGNAL(clicked(bool)),this,SLOT(ChangeSleeperVisible(bool))	);
	connect( ui.changeSpeed	,SIGNAL(clicked(bool)),this,SLOT(ChangePhysis(bool))	);
	connect( ui.sleeperFrqt	,SIGNAL(valueChanged(int)),this,SLOT(ChangeSleeperFrqtOfTrain(int))	);
	connect( ui.tension		,SIGNAL(valueChanged(int)),this,SLOT(ChangeSplineTension(int))	);
	connect( ui.subdivision	,SIGNAL(valueChanged(int)),this,SLOT(ChangeSplineSubdivision(int))	);
...
void AppMain::ChangeTrackStyle(QString type)
{
	if (type == "ClassicTrain")
	{
		this->trainview->style = 0;
	}
	else if (type == "Simple")
	{
		this->trainview->style = 1;
	}
	else if (type == "RollerCoaster")
	{
		this->trainview->style = 2;
	}
}
...
void AppMain::SwitchPlayAndPause()
{
	...
	{
		ui.bPlay->setIcon(QIcon(":/AppMain/Resources/Icons/pause.png"));
		this->trainview->isrun = !this->trainview->isrun;
	}
	else
	{
		ui.bPlay->setIcon(QIcon(":/AppMain/Resources/Icons/play.png"));
		this->trainview->isrun = !this->trainview->isrun;
	}
	if(this->trainview->isrun){
...
void AppMain::MoveBackWord()
{
	this->trainview->m_pTrack->trainU -= 2;
	if (this->trainview->m_pTrack->trainU < 0)
		this->trainview->m_pTrack->trainU = this->trainview->m_pTrack->totalLen + this->trainview->m_pTrack->trainU;
}

void AppMain::MoveForWord()
{
	this->trainview->m_pTrack->trainU += 2;
	if (this->trainview->m_pTrack->trainU > this->trainview->m_pTrack->totalLen)
		this->trainview->m_pTrack->trainU -= this->trainview->m_pTrack->totalLen;
}

void AppMain::AddCar()
{
	this->trainview->m_train->carNum++;
}

void AppMain::RemoveCar()
{
	if(this->trainview->m_train->carNum > 0)
		this->trainview->m_train->carNum--;
}

void AppMain::MVViewPosForward()
{
	if(this->trainview->m_pTrack->standCarIndex >= 0)
		this->trainview->m_pTrack->standCarIndex--;
}

void AppMain::MVViewPosBackward()
{
	if (this->trainview->m_pTrack->standCarIndex < this->trainview->m_train->carNum)
		this->trainview->m_pTrack->standCarIndex++;
}
...
void AppMain::ChangeSpeedOfTrain( int val )
{
	...
	this->trainview->m_pTrack->speed = val;
	//m_rollerCoaster->trainSpeed = m_rollerCoaster->MAX_TRAIN_SPEED * float(val) / 100.0f;
}

void AppMain::ChangeSleeperVisible(bool checked)
{

	this->trainview->m_pTrack->sleeperVisible = checked;
	//m_rollerCoaster->trainSpeed = m_rollerCoaster->MAX_TRAIN_SPEED * float(val) / 100.0f;
}

void AppMain::ChangePhysis(bool checked)
{

	this->trainview->m_pTrack->physics = checked;
	//this->trainview->m_pTrack->speed = ui.sSpeed->value;
}

void AppMain::ChangeSleeperFrqtOfTrain(int val)
{

	this->trainview->m_pTrack->sleeperFrqt = 16 - val;
	//m_rollerCoaster->trainSpeed = m_rollerCoaster->MAX_TRAIN_SPEED * float(val) / 100.0f;
}

void AppMain::ChangeSplineTension(int val)
{

	this->trainview->m_pTrack->tension = val / 100.f;
}

void AppMain::ChangeSplineSubdivision(int val)
{

	this->trainview->m_pTrack->subdivision = val;
}
void AppMain::AddControlPoint()
{
	...
	glm::vec3 npos = (this->m_Track.points[previdx].pos + this->m_Track.points[newidx].pos) * .5f;
...
void AppMain::rollx(float dir)
{
	...
	if (s >= 0) {
		glm::vec3 old = this->m_Track.points[s].orient;
...
void AppMain::ChangeStyleToClassic()
{
	this->trainview->style = 0;
}

void AppMain::ChangeStyleToSimple()
{
	this->trainview->style = 1;
}

void AppMain::ChangeStyleToRoller()
{
	this->trainview->style = 2;
}
...
void AppMain::UpdateVelocityState(int index)
{
	this->trainview->m_pTrack->speed = index / 10;
}
